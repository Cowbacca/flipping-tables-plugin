...
- Gradle **8.10**; Flipping Tables plugin **2.1.3**.
...
2. Open the Flipping Tables sidebar, check the API destination shown there, and set the hours until you next return (decimals such as `2.5` work). Advice always evaluates eight-hour buying and selling windows; your return time only counts buy-limit resets available before you return. Outstanding buy reserves are captured separately.
3. Leave **Sell all carried items** on to include every tradeable item in your inventory. If you want to choose items individually, turn it off; the plugin reads your portfolio and shows **Inventory to sell**. **Already selling** remains included automatically. Inventory choices and optional costs survive a portfolio refresh within the current session. Unselected carried stock, uncollected items and bank stock are excluded. Optional item costs default to zero/unknown.
4. Press **Plan next visit**. The plugin reads the current portfolio and requests advice. Leave the cash budget blank to use all carried coins, or enter a smaller amount. The button sends the portfolio data described in the sidebar to the API.
5. Review keep/cancel/reprice/create suggestions and apply them manually. Cancelling or collecting an offer keeps the saved advice visible. Press **Plan next visit** again to include newly collected stock in a fresh plan. For new buys, type `ft` in the GE search or click its coins button. Choose a suggested item, open the normal quantity or price entry, and click **Use suggested quantity** or **Use suggested price** beneath the input. Press Enter to accept the number, then review and confirm the offer normally. A unique matching suggestion is selected automatically; **Use this suggestion** in the sidebar resolves multiple recommendations for the same item. **Copy quantity** and **Copy price** are also available.
6. A new offer placed with exactly the suggested item, side, quantity and price is marked **Placed**. Remaining suggestions stay available, including when that offer fills immediately. Reopening buy search restores the suggested list if you were using it; typing a normal item search or clicking the coins button switches back to normal search. The plugin never hides the shared text input or submits an offer.
...
Amounts are estimates, not realised profit. The panel shows expected profit only if the portfolio API returns it. The current portfolio API reports an inventory value estimate even when costs are unknown, and discloses when its search budget is exhausted. It does not know queue position or future prices.
